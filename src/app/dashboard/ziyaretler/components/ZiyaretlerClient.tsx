...
export default function ZiyaretlerClient({ initialVisits, initialFirms }: { initialVisits: any[], initialFirms: any[] }) {
  ...
  const handleDelete = async () => {
    if (!deleteId) return
    try {
      const res = await fetch(`/api/ziyaretler/${deleteId}`, { method: 'DELETE' })
      if (res.ok) { setDeleteId(null); fetchVisits() }
              ...
              {isLoading ? (
                ...
                filtered.map((visit) => (
                  <tr key={visit.id}>
                    ...
                    <td><span className="badge badge-info">{VISIT_TYPE_LABELS[visit.visitType as "PERIODIC_CONTROL" | "EMERGENCY" | "RISK_ASSESSMENT" | "TRAINING" | "ACCIDENT_INVESTIGATION" | "ISG_COMMITTEE" | "OTHER"]}</span></td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4 }}>
                        ...
                        {can('VISIT_DELETE') && (
                          <button className="btn btn-sm btn-ghost" title="Sil" onClick={() => setDeleteId(visit.id)} style={{ color: 'var(--risk-critical)' }}>
      ...
      {deleteId && (
        <div className="modal-overlay" onClick={() => setDeleteId(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            <div className="modal-header"><h3 className="modal-title">Ziyaret Silme</h3><button className="modal-close" onClick={() => setDeleteId(null)}><Icon name="X" size={20} /></button></div>
            <div className="modal-body"><div className="alert alert-warning"><Icon name="AlertTriangle" size={18} /><div>Bu ziyaret kaydı kalıcı olarak silinecektir.</div></div></div>
